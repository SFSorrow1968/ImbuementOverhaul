...
5. **Push**: Always push immediately after committing.
   - Example: `git push -u origin agent/<topic>`
6. **Version Tag**: Create/update a tag pointing to this snapshot.
   - Example: `git tag -f v1.0.0-snapshot`
   - Always push tags: `git push origin <tag>`
7. **Merge**: Merge to `main` only after validation.
